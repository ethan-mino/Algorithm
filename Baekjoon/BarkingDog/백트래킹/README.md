...
예를 들어 아래와 같은 함수는 depth부터 시작하여 총 m개를 선택하는 모든 경우의 수를 탐색하는 함수로 정의할 수 있습니다.  
back 함수는 depth번째 단계에서 원소를 하나 선택하고, back(depth + 1, m)를 호출하여, 0부터 시작하여 총 m개를 선택하는 함수가 될 수 있습니다.  

    /*
        * @param m 선택해야 할 원소의 총 개수
        * m은 반드시 backtracking 함수의 인자로 전달될 필요는 없습니다, static 변수로 두어도 됩니다
        * @param depth 현재까지 선택한 원소의 개수
        * depth는 main 함수 호출 시 0또는 1로 시작할 수 있습니다. 다만, 저는 0을 사용합니다.
    */
    ...
    void back(int depth, int m){
        if(depth == m){
            ...
            for(int cur = XXX; cur < XXX ; cur++){
                // 2. 원소를 선택하는 부분
                back(depth + 1, m); // depth + 1부터 총 m개를 선택하는 함수를 호출
            }
        }
    }

	public static void main(String[] args) {
        back(0, 10); // depth 0부터 시작하여 10개를 선택하는 모든 경우의 수를 탐색
...
depth가 기준이 되는 유형은 현재 단계에서 원소를 선택하고, 다음 단계를 호출해주면 됩니다.  
    ...
    */
    ...
    static void backtracking(int depth, int m, int preDist){
        if(depth == m){    
            ...
            minDist = Math.min(minDist, preDist);
        }else{  // 아직 m개를 선택하지 않은 경우
            // 선택 가능한 원소들 중에서 하나를 선택
...
## 풀이 과정
    ...
    2. n의 크기를 살펴봄
        2-1. 백트래킹 하기엔 n이 너무 큰 경우, 다른 유형을 시도 
        2-2. 백트래킹의 경우 보통 10이하의 n이 주어짐, 그 이상은 가지치기를 고려
        2-2. 그 이상의 경우, DP/그리디/이분탐색을 고려
...
## ⚠ 주의할 점
최종 출력 변수의 값을 업데이트 할 수 있는 존재하는 경우의 수가 전혀 없는 경우,   
Integer.MAX_VALUE 또는 Integer.MIN_VALUE와 같은 쓰레기 값이 출력될 수 있습니다.  
따라서, 이러한 값이 나올 수 있는지, 경우의 수가 존재하지 않을 때, 출력해야 될 값을 문제에서 제시하였는지 등을 확인해야합니다.
