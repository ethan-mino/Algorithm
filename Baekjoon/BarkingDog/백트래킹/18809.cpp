...
void bfs(vector<pair<int, int>>& gv, vector<pair<int, int>>& rv){
    ...
    for(auto c : gv) {q.push({1, c});  vis[c.x][c.y].u = 1; vis[c.x][c.y].dist = 1;};  // 초록색 배양액을 queue에 넣고 방문 표시 (초록색 배양액의 고유번호는 1)
    for(auto c : rv) {q.push({2, c}); vis[c.x][c.y].u = 2; vis[c.x][c.y].dist = 1;};   // 빨간색 배양액을 queue에 넣고 방문 표시 (빨간색 배양액의 고유번호는 2)
    ...
    while(!q.empty()){
        ...
        if(vis[cur.pos.x][cur.pos.y].u == 3) continue;   // 꽃을 피운 위치는 더이상 배양액을 퍼트리지 않도록 함.

        for(int dir = 0; dir < 4; dir++){
            int nx = cur.pos.x + dx[dir];
            int ny = cur.pos.y + dy[dir];

            if(nx < 0 || ny < 0 || nx >= n || ny >= m) continue;    // 인접한 좌표가 정원의 인덱스를 벗어나는 경우 continue
            if(board[nx][ny] == 0) continue;    // 인접한 좌표가 호수인 경우

            if(vis[nx][ny].u == 0){   // 방문하지 않은 경우
                q.push({cur.u, {nx, ny}});
                vis[nx][ny].dist = vis[cur.pos.x][cur.pos.y].dist + 1;    // 동일한 시간에 도달했는 지 확인하기 위해 방문 표시 배열에 dist도 저장
                vis[nx][ny].u = cur.u;  // 어떤 배양액으로 부터 퍼진 것인지 표시
            }else if((vis[nx][ny].dist == vis[cur.pos.x][cur.pos.y].dist + 1) && 
            (vis[nx][ny].u == 1 && cur.u == 2 || vis[nx][ny].u == 2 && cur.u == 1)){  // 초록색 배양액과 빨간색 배양액이 동일한 시간에 도달한 경우
...
int main(){
    ...
    vector<int> pg, pr; // next_permutation을 이용하여 조합을 수행하기 위한 배열 ex. 5C3의 경우 {0, 0, 1, 1, 1}
    ...
    do{ // 먼저 전체 배양액을 뿌릴 수 있는 땅 중 초록색 배양액을 뿌릴 땅을 선택 (combination)
        ...
        for(int i = 0; i < pos.size(); i++)
            if(pg[i] == 1) gv.push_back(pos[i]);    // 선택된 빨간색 배양액과 그에 해당하는 좌표를 매핑시켜준다. 

        do{ // 배양액을 뿌릴 수 있는 땅 중 초록색 배양액을 뿌릴 땅은 제외하고 빨간색 배양액을 뿌릴 땅을 선택한다. (combination)
            vector<pair<int, int>> rv;

            for(int i = 0; i < pr.size(); i++){ // 선택된 초록색 배양액과 그에 해당하는 좌표를 매핑시켜준다. 
                if(pr[i] != 1) continue;
                int zero_th = 0;
            ...
            bfs(gv, rv);    // 선택된 초록색 배양액과 빨간색 배양액의 좌표를 넘겨 BFS를 수행하고, 해당 좌표를 선택했을 때, 피울 수 있는 꽃의 최대 개수를 계산한다. 
